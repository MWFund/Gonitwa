window.onload = function(){
	...
	function movePlayer(row,col){
		...
		if(col != oldCol || row != oldRow){
			// Safety check - ensure cells exist
			if(!checkBoard[oldRow] || !checkBoard[oldRow][oldCol]){
				console.error('Invalid old cell:', oldRow, oldCol);
				return;
			}
    ...
    function processInput(){
        ...
        if(!player.anim && mouse.x > 0 && mouse.x<SIZE && mouse.y > HORIZON_Y && mouse.y < SIZE + HORIZON_Y ){
            ...
            // Check if reverseProject gave valid results
            if(isNaN(mouseCol) || isNaN(mouseRow)){
                mouseCol = -1;
                mouseRow = -1;
            }else{
	...
	document.onmousemove = function(e){
		...
		mouse.y = e.clientY - root.offsetTop;
	};

	function onmouse(isClick,e){
		mouse.click = isClick;
		if(e) document.onmousemove(e);
	}
	...
	if(speedSlider){
		speedSlider.addEventListener('input', function(e){
			...
			progressPerSec = speed / 5; // Scale: 1-10 slider -> 0.2-2.0 speed
